...
def dibujar_teclado(frame, tecla_actual, sugerencia):
    h, w, _ = frame.shape
    ...
    # Mostrar sugerencia
    if sugerencia:
        cv2.putText(frame, f"Sugerencia: {sugerencia}", (x_inicio, y_inicio - 20),
...
def obtener_sugerencia(texto):
    if len(texto.strip()) == 0:
        return ""
    try:
        salida = modelo(texto, max_length=len(texto) + 4, num_return_sequences=1, do_sample=True)
        sugerido = salida[0]["generated_text"][len(texto):].split()[0]
        return sugerido
    except Exception:
        return ""
...
while True:
    ...
    dibujar_teclado(frame, tecla_actual, sugerencia)
    cv2.imshow("Teclado Visual + Predicción", frame)
