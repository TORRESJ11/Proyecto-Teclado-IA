...
# Dimensiones del teclado visual
tamaño_celda = 60
margen = 20
...
tiempo_confirmacion = 1.0  # segundos que debes mirar antes del parpadeo

# Función para dibujar el teclado sobre la imagen
def dibujar_teclado(frame, tecla_actual):
    ...
    for i, fila in enumerate(teclas):
        for j, tecla in enumerate(fila):
            ...
            color = (255, 0, 0) if (i, j) == tecla_actual else (50, 50, 50)
            cv2.rectangle(frame, (x, y), (x + tamaño_celda, y + tamaño_celda), color, -1)
            cv2.putText(frame, tecla, (x + 15, y + 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

# Función para detectar parpadeo (diferencia entre párpado superior e inferior)
def detectar_parpadeo(puntos, ojo_ids):
    arriba = puntos[ojo_ids[1]]
    abajo = puntos[ojo_ids[5]]
    distancia = abs(arriba.y - abajo.y)
    return distancia < 0.015  # Umbral empírico

# Función para determinar la tecla enfocada por la mirada
...
while True:
    ...
    if resultado.multi_face_landmarks:
        ...
        # Confirmación de selección por parpadeo
        if tecla_actual:
    ...
    cv2.putText(frame, texto_escrito, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 2)
    dibujar_teclado(frame, tecla_actual)
    cv2.imshow("Teclado Visual con la Mirada", frame)
